...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.each do |season, day|
    puts "#{season.capitalize}:"
      ...
      i = 1
      holiday_names_array = holiday.to_s.split("_")
      holiday_names = holiday_names_array.collect do |day|
        day.capitalize
      end
      special_day = holiday_names.join(" ")
      supply_list = "  #{special_day}: #{supplies[0]}"
        while i < supplies.length
          supply_list = supply_list + ", #{supplies[i]}"
          i += i
        end
        puts supply_list
      end
